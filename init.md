# Prerequisites
Before anything: 
- Ensure `agent-browser` is installed and it can be used.
- Ensure there is a `scope.md` file in this same level.
  - It must define a target Website
  - It must define a list of steps to follow
  - It must define a list of personas
...
For that, certain steps need to be followed. It is important to note that in order to perform them, no previous attempts must be taken into consideration AT ALL. All you care about is this file and `scope.md`.
...
## Personas Folder
- For each of the personas listed [here](scope.md#personas), create a Markdown file in `00 - Personas` that will describe how the persona behaves interacting with the website. Feel free to spin up a Subagent to write up each of these personas.
